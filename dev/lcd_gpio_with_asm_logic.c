...
void send_message_to_lcd() {
    ...
    // Send the initialization command sequence to the LCD
    // Assuming the LCD module has been initialized previously
    // Write data to the LCD (e.g., write the message "Button Pressed")
    for (int i = 0; i < sizeof(message) - 1; i++) {
        if (write(i2c_fd, &message[i], 1) != 1) {
            perror("Failed to write to I2C device");
            close(i2c_fd);
            return;
        }
        delay_ms(1);  // Small delay for each character
    }
...
int main() {
    ...
    // Hand control to the assembly code, passing GPIO base address as argument
