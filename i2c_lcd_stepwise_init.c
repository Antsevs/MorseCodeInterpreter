...
void send_command(unsigned char command) {
    ...
    // Write command to FIFO (RS bit = 0 for command, add 0x08 for backlight on)
    i2c[I2C_FIFO / 4] = command | 0x08;
...
void send_data(unsigned char data) {
    ...
    // Write data to FIFO (RS bit = 1 for data, add 0x08 for backlight on)
    i2c[I2C_FIFO / 4] = data | 0x09;
...
int main() {
    ...
    // Step 1: Send Display Control Command (0x0C) to turn on display, cursor off, backlight on
    printf("Sending Display Control Command (0x0C)...\n");
    send_command(0x0C);
    delay_ms(5000);  // Wait and observe if the LCD remains on with backlight

    // Step 2: Send Function Set Command (0x28) for 2 lines, 5x8 dots, with backlight on
    printf("Sending Function Set Command (0x28)...\n");
    send_command(0x28);
    delay_ms(5000);  // Wait and observe if the LCD remains on with backlight

    // Step 3: Send Entry Mode Set Command (0x06), with backlight on
    printf("Sending Entry Mode Set Command (0x06)...\n");
    send_command(0x06);
    delay_ms(5000);  // Wait and observe if the LCD remains on with backlight

    printf("Initialization Completed Successfully.\n");

    // Write "Hello" to the LCD, with backlight on
    char message[] = "Hello";
    for (int i = 0; i < 5; i++) {
        send_data(message[i]);
    }
